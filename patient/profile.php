...
<head>
    ...
    <title>Patient Telemetry Profile | CARE Nexus</title>
...
<body>
    <div class="dash-container">
        <aside class="dash-sidebar">
            ...
            <nav class="dash-nav">
                <a href="dashboard.php">❖ My Care HUD</a>
                <a href="../find_doctor.php">❖ Holographic Search</a>
                <a class="active" href="profile.php">❖ Patient Telemetry</a>
        ...
        <main class="dash-content">
            <header class="section-heading">
                <div>
                    <p class="eyebrow">ACCOUNT TELEMETRY SETTINGS</p>
            ...
            <section class="cyber-table-wrap" style="max-width: 860px;">
                <form method="post" enctype="multipart/form-data" style="display: grid; gap: 20px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 24px; padding-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.08);">
                        <?php if($img): ?>
                            <img src="<?=htmlspecialchars($img)?>" onerror="this.src='https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=300'" style="width: 100px; height: 100px; border-radius: 20px; border: 3px solid var(--cyan-neon); object-fit: cover;" alt="Profile">
                        <?php else: ?>
                            <div style="width: 100px; height: 100px; border-radius: 20px; border: 3px solid var(--border-cyber); background: rgba(0,242,254,0.08); display: flex; align-items: center; justify-content: center; font-size: 32px; color: var(--cyan-neon);">❖</div>
                        <?php endif; ?>
                        <div class="field" style="flex: 1;">
                            <label>CHANGE PROFILE SHARD IMAGE</label>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 18px;">
                        <div class="field">
                            ...
                            <input name="phone" value="<?=htmlspecialchars($patient['phone'])?>" required>
                        </div>
                        <div class="field">
                            <label>CITY NODE <span style="color: var(--cyan-neon);">*</span></label>
                            <select name="city_id" required>
                                <?php while($city = mysqli_fetch_assoc($cities)): ?>
                                    <option value="<?=$city['city_id']?>" <?=$patient['city_id']==$city['city_id']?'selected':''?>><?=htmlspecialchars($city['city_name'])?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="field">
                        <label>RESIDENTIAL ADDRESS <span style="color: var(--cyan-neon);">*</span></label>
                        <input name="full_address" value="<?=htmlspecialchars($patient['full_address'])?>" required>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 18px;">
                        <div class="field">
                            ...
                            <input name="emergency_contact_name" value="<?=htmlspecialchars($patient['emergency_contact_name'])?>">
                        </div>
                        <div class="field">
                            <label>EMERGENCY PHONE NUMBER</label>
                            <input name="emergency_contact_phone" value="<?=htmlspecialchars($patient['emergency_contact_phone'])?>">
                        </div>
                    </div>

                    <button class="btn btn-primary" name="update_profile" style="width: 100%; margin-top: 10px;">
                        <span>❖ Save Patient Telemetry Profile</span>
                    </button>
                </form>
            </section>
        </main>
